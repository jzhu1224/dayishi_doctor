...
public class EaseVoiceRecorderView extends RelativeLayout {
    ...
    private boolean isVoiceRecordComplete = false;
    ...
    protected Handler micImageHandler = new Handler() {
        ...
        public void handleMessage(android.os.Message msg) {
            ...
            int recordLength = voiceRecorder.getRecordSeconds();
            Log.d("zhujiang","recodeLength:"+recordLength);
            if (recordLength >= 60) { //超过最长时间限制 停止录制
                Log.d("zhujiang","最长限制:");
                int length = voiceRecorder.stopRecoding();
                if (recorderCallback != null) {
                    recorderCallback.onVoiceRecordComplete(getVoiceFilePath(), length);
                }
                isVoiceRecordComplete = true;
                EaseVoiceRecorderView.this.setVisibility(View.GONE);
            }
        }
    };
    ...
    public void startRecording() {
        ...
        try {
            ...
            recordingHint.postDelayed(new Runnable() {
                @Override
                public void run() {
